...
/** Stable UUID-shaped identifier that works identically in Electron main and browser renderer. */
export function stablePresetEntityId(value: string): string {
  const bytes = sha256(new TextEncoder().encode(value)).slice(0, 16);
